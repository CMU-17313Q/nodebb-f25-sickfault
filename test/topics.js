...
describe('Topic\'s', () => {
	...
	describe('tools/delete/restore/purge', () => {
		...
		/**
		 * Test suite for mark-resolved feature acceptance criteria
		 *
		 * This test suite validates all acceptance criteria for the mark-resolved feature:
		 *
		 * 1. "Mark as Resolved" button on topic page
		 *    - Tests that topics can be marked as resolved via API (simulating button click)
		 *    - Verifies resolved/resolvedBy/resolvedAt fields are properly set
		 *
		 * 2. Updates UI without page refresh
		 *    - Tests that topic data is immediately updated after marking as resolved
		 *    - Verifies data changes are reflected without requiring page reload
		 *
		 * 3. Green checkmark icon on resolved topics
		 *    - Tests that resolved flag is included in topic data for UI rendering
		 *    - Verifies the flag is used to display visual indicators in the frontend
		 *
		 * 4. Filter option to show/hide resolved topics
		 *    - Tests 'resolved' filter shows only resolved topics
		 *    - Tests 'unresolved' filter shows only unresolved topics
		 *    - Tests no filter shows all topics (both resolved and unresolved)
		 */
		describe('Mark as Resolved Feature - Acceptance Criteria Tests', () => {
			let testCategory;
			let resolvedTopic;
			...
			before(async () => {
			// Create a test category for filtering tests
				testCategory = await categories.create({
					name: 'Resolved Filter Test Category',
					description: 'Category for testing resolved topic filters',
				});

				// Create test topics - one resolved, two unresolved
				const resolved = await topics.post({
					uid: adminUid,
					title: 'Resolved Test Topic',
					content: 'This topic will be marked as resolved',
					cid: testCategory.cid,
				});
				resolvedTopic = resolved.topicData;

				const unresolved1 = await topics.post({
					uid: adminUid,
					title: 'Unresolved Test Topic 1',
					content: 'This topic remains unresolved',
					cid: testCategory.cid,
				});
				unresolvedTopic1 = unresolved1.topicData;

				const unresolved2 = await topics.post({
					uid: fooUid,
					title: 'Unresolved Test Topic 2',
					content: 'Another unresolved topic',
					cid: testCategory.cid,
				});
				unresolvedTopic2 = unresolved2.topicData;

				// Mark first topic as resolved
				await apiTopics.resolve({ uid: adminUid }, { tids: [resolvedTopic.tid] });
			});

			// Acceptance Criteria 1 & 2: "Mark as Resolved" button functionality and UI updates without page refresh
			it('should mark topic as resolved and update resolved fields correctly', async () => {
			// Create a new topic to test marking as resolved
				const result = await topics.post({
					uid: adminUid,
					title: 'Test Resolve Button',
					content: 'Testing mark as resolved button',
					cid: testCategory.cid,
				});

				// Mark the topic as resolved (simulating button click)
				await apiTopics.resolve({ uid: adminUid }, { tids: [result.topicData.tid] });

				// Verify the topic data is updated (this would reflect in UI without refresh)
				const topicData = await topics.getTopicFields(result.topicData.tid, ['resolved', 'resolvedBy', 'resolvedAt']);

				assert.strictEqual(topicData.resolved, 1, 'Topic should be marked as resolved');
				assert.strictEqual(topicData.resolvedBy, adminUid, 'resolvedBy should be set to the user who resolved it');
				assert(topicData.resolvedAt > 0, 'resolvedAt timestamp should be set');
			});

			// Acceptance Criteria 3: Green checkmark icon on resolved topics
			it('should include resolved flag in topic data for UI indicator display', async () => {
			// Get topic data as it would be returned to the frontend
				const topicData = await topics.getTopicData(resolvedTopic.tid);

				// Verify resolved flag is present (used to show green checkmark in UI)
				assert.strictEqual(topicData.resolved, 1, 'Resolved topic should have resolved flag set to 1');
				assert(topicData.resolvedBy, 'Resolved topic should have resolvedBy field set');

				// Verify unresolved topic does not have the flag
				const unresolvedData = await topics.getTopicData(unresolvedTopic1.tid);
				assert.strictEqual(unresolvedData.resolved, 0, 'Unresolved topic should have resolved flag set to 0');
			});

			// Acceptance Criteria 4: Filter option to show/hide resolved topics
			it('should filter topics by resolved status - show only resolved topics', async () => {
			// Get category topics with 'resolved' filter
				...
				// Verify only resolved topics are returned
				assert(result.topics.length > 0, 'Should return at least one resolved topic');
				result.topics.forEach((topic) => {
					assert.strictEqual(topic.resolved, 1, 'All returned topics should be resolved');
				});

				// Verify our specific resolved topic is in the results
				const foundResolved = result.topics.find(t => t.tid === resolvedTopic.tid);
				assert(foundResolved, 'Resolved topic should be in filtered results');

				// Verify unresolved topics are NOT in the results
				const foundUnresolved1 = result.topics.find(t => t.tid === unresolvedTopic1.tid);
				const foundUnresolved2 = result.topics.find(t => t.tid === unresolvedTopic2.tid);
				assert(!foundUnresolved1, 'Unresolved topics should not be in resolved filter results');
				assert(!foundUnresolved2, 'Unresolved topics should not be in resolved filter results');
			});

			it('should filter topics by resolved status - show only unresolved topics', async () => {
			// Get category topics with 'unresolved' filter
				const result = await categories.getCategoryTopics({
					cid: testCategory.cid,
					start: 0,
					stop: 19,
				...
				// Verify only unresolved topics are returned
				assert(result.topics.length > 0, 'Should return at least one unresolved topic');
				result.topics.forEach((topic) => {
					assert.notStrictEqual(topic.resolved, 1, 'All returned topics should be unresolved');
				});

				// Verify our specific unresolved topics are in the results
				const foundUnresolved1 = result.topics.find(t => t.tid === unresolvedTopic1.tid);
				const foundUnresolved2 = result.topics.find(t => t.tid === unresolvedTopic2.tid);
				assert(foundUnresolved1, 'Unresolved topic 1 should be in filtered results');
				assert(foundUnresolved2, 'Unresolved topic 2 should be in filtered results');

				// Verify resolved topic is NOT in the results
				const foundResolved = result.topics.find(t => t.tid === resolvedTopic.tid);
				assert(!foundResolved, 'Resolved topic should not be in unresolved filter results');
			});

			it('should show all topics when no filter is applied', async () => {
			// Get category topics without filter
				const result = await categories.getCategoryTopics({
					cid: testCategory.cid,
					start: 0,
					stop: 19,
				...
				// Verify all topics are returned (both resolved and unresolved)
				assert(result.topics.length >= 3, 'Should return all topics when no filter is applied');

				const foundResolved = result.topics.find(t => t.tid === resolvedTopic.tid);
				const foundUnresolved1 = result.topics.find(t => t.tid === unresolvedTopic1.tid);
				const foundUnresolved2 = result.topics.find(t => t.tid === unresolvedTopic2.tid);

				assert(foundResolved, 'Resolved topic should be in unfiltered results');
				assert(foundUnresolved1, 'Unresolved topic 1 should be in unfiltered results');
				assert(foundUnresolved2, 'Unresolved topic 2 should be in unfiltered results');
			});

			// Additional test: Verify unresolve functionality updates UI data correctly
			it('should unresolve topic and update UI data correctly', async () => {
			// Create and resolve a topic
				const result = await topics.post({
					uid: adminUid,
					title: 'Test Unresolve Button',
					content: 'Testing unresolve button',
					cid: testCategory.cid,
				});
				await apiTopics.resolve({ uid: adminUid }, { tids: [result.topicData.tid] });

				// Unresolve the topic (simulating button click)
				await apiTopics.unresolve({ uid: adminUid }, { tids: [result.topicData.tid] });

				// Verify the topic data is updated (this would reflect in UI without refresh)
				const topicData = await topics.getTopicFields(result.topicData.tid, ['resolved', 'resolvedBy', 'resolvedAt']);

				assert.strictEqual(topicData.resolved, 0, 'Topic should be marked as unresolved');
				assert.strictEqual(topicData.resolvedBy, 0, 'resolvedBy should be cleared');
				assert.strictEqual(topicData.resolvedAt, 0, 'resolvedAt should be cleared');
			});
		});
